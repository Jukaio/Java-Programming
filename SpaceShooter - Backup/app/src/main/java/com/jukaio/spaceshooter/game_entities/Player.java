...
import com.jukaio.spaceshooter.component.Weapons_Component;
...
public class Player extends Clamping_Entity
{
    ...
    Weapons_Component m_weapon_pool = null;
    float m_power_up_timer = 0;
    float p_power_up_duration = 0;
    
    private Vector2 prev_velocity = Vector2.ZERO;
    ...
    Bitmap[] damage = new Bitmap[NUM_DAMAGE_SPRITES];
    
    // Resource to float
    static float fr(int p_resource_id)
    {
        TypedValue outValue = new TypedValue();
        m_game.getResources().getValue(p_resource_id, outValue, true);
    ...
    public Player(boolean p_active, int p_bitmap_id, Player_Data p_data)
    {
        ...
        // damage bitmaps
        damage[0] = Utility.load_bitmap(m_game, R.drawable.damage_0);
        damage[0] = Utility.resize_bitmap(damage[0], p_data.m_scale);
        damage[1] = Utility.load_bitmap(m_game, R.drawable.damage_1);
        damage[1] = Utility.resize_bitmap(damage[1], p_data.m_scale);
        damage[2] = Utility.load_bitmap(m_game, R.drawable.damage_2);
        damage[2] = Utility.resize_bitmap(damage[2], p_data.m_scale);
        m_bitmap_component = new Bitmap_Component(m_game, this, p_bitmap_id, p_data.m_scale,
                                                   0xFFFFFFFF);
        m_weapon_pool       = new Weapons_Component(this, p_data);
        m_acceleration      = p_data.m_accel;
        m_deceleration      = p_data.m_deaccel;
        m_max_velocity      = p_data.m_max_vel;
        m_max_hp            = m_game.getResources().getInteger(R.integer.max_hp);
        m_hp                = m_max_hp;
    ...
    public void render(Canvas p_canvas, Paint p_paint)
    {
        ...
        if(m_hp < m_max_hp)
            p_canvas.drawBitmap(damage[NUM_DAMAGE_SPRITES - m_hp],
                                       get_rectangle().left,
                                       get_rectangle().top,
                                       p_paint);
    }
    ...
    private void on_power_up_collision(Power_Up_Type p_type)
    {
        ...
        {
            case DUAL_GUN:
                m_weapon_pool.activate_dual_gun(p_power_up_duration);
