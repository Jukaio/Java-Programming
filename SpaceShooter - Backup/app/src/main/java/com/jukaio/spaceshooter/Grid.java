    ...
    public void draw(Canvas p_canvas, Paint p_paint)
    {
        for(int x = 0; x < m_grid_width; x++)
        {
            for (int y = 0; y < m_grid_height; y++)
            {
                ...
                p_canvas.drawText(Integer.toString(x) + " " + Integer.toString(y), x * m_tile_width + (m_tile_width * 0.5f) + m_offset.m_x, y * m_tile_height + (m_tile_height * 0.5f) + m_offset.m_y, p_paint);
